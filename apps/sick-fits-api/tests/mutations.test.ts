...
const asUser = (context: Context, itemId?: string) =>
...
describe(`Custom mutations`, () => {
  describe('checkout(token)', () => {
    ...
    test('Not logged in should throw', async () => {
    ...
    test('A users cart should correctly convert into an order', async () => {
      jest
        .spyOn(stripeConfig.paymentIntents, 'create')
  ...
  describe('addToCart(productId)', () => {
    ...
    test('Adding a non-existant product should throw', async () => {
    ...
    test('Adding a null product should throw', async () => {
      const { graphql } = asUser(context, FAKE_ID);
      const { data, errors } = await graphql.raw({
    ...
    test('Adding DRAFT product should throw', async () => {
    ...
    test('Adding UNAVAILABLE product should throw', async () => {
      const { User, Product } = context.sudo().query;
      // Setup user
      const user = await User.createOne({
        data: { name: 'Test User', email: 'test@example.com' },
    ...
    test('Adding AVAILABLE product should return a cart item with a quantity of 1', async () => {
      const { User, Product } = context.sudo().query;
      // Setup user
      const user = await User.createOne({
        data: { name: 'Test User', email: 'test@example.com' },
    ...
    test('Adding a product multiple times should return a cart item with the correct quantity', async () => {
      const { User, Product } = context.sudo().query;
      // Setup user
      const user = await User.createOne({
        data: { name: 'Test User', email: 'test@example.com' },
    ...
    test('Adding different products multiple times should return cart items with the correct quantity', async () => {
      const { User, Product } = context.sudo().query;
      // Setup user
      const user = await User.createOne({
        data: { name: 'Test User', email: 'test@example.com' },
